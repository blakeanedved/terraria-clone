...
void Worm::SetMaxLength(){
	double value = pnoise.GetValue((double)x / scale, (double)y / scale, ((double)x * (double)y) / scale);
	if (value < -1) value = -1;
	if (value > 1) value = 1;
	max_length = ((value + 1) / 2) * (WORM_MAX_LENGTH - WORM_MIN_LENGTH) + WORM_MIN_LENGTH;
}
...
void Worm::Slither(){
	...
	for (int i = 0; i < max_length; i++){
		...
		if (dir < 1.4){ // if the direction is less than relatively straight down
			// weight the direction towards 1.1 to straighten the worm out
			dir = std::max(1.1, dir + (1.1 - dir) * side_weight);
		} else if (dir > 1.6){ // if the direction is greater than relatively straight down
